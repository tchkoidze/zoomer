  ...
  const tabItems = categories.map((cat: ProductCategory, index: number) => {
    ...
    return (
      ...
      <div
        ...
        } border-solid border-border-white dark:border-border-dark-white flex items-center w-full cursor-pointer hover:bg-white-400 dark:hover:bg-dark-black-06 transition-colors duration-300 ease-in-out px-[10px] first:rounded-t-xl last:rounded-b-xl`}
        onClick={() => navigate(`/products/${cat.name}`)}
      >
        {cat.image && cat.image !== "data:image/png;base64," && (
          <img
            src={cat.image}
            alt="category icon"
            className="w-6 h-5 object-contain mr-3"
          />
        )}
        <p
