  ...
  return (
    <div className="col-span-1 w-[140px] md:w-[165px] h-[300px] flex flex-col justify-between relative mx-auto">
      <img
        ...
        src={product.image}
        alt="product image"
        className="w-full h-[165px] object-cover cursor-pointer"
      />
      <div
        className={`${
          authStage === AuthStageEnum.AUTHORIZED ? "flex" : "hidden"
        } w-7 h-7 bg-white-400 rounded-[50%] absolute top-1 right-1 items-center justify-center`}
      >
        {addLoading || removeLoading ? (
          <LoadingSpinner size={28} fullscreen={false} custom={true} />
        ) : (
          <img
            src={isInWishlist(product.id) ? wishlistIconActive : wishlistIcon}
            alt="wishlist icon"
      ...
      <div>
        {!product.salePrice && (
          <p className="firago-semibold text-base leading-[19px] mb-3 text-black-main dark:text-dark-black-main">
            {product.price} ₾
          </p>
        )}
        {product.salePrice && (
          <div className="flex">
            <p className="firago-semibold text-base leading-[19px] mb-3 text-orange-main dark:text-text-dark-orange-main mr-2">
              {product.salePrice} ₾
            </p>
        ...
        <h3
          ...
          className="max-w-40 firago-medium text-xs leading-[18px] opacity-80 cursor-pointer text-black-main dark:text-dark-black-main"
        >
          {product.title}
        </h3>
      </div>
