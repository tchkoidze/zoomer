...
export default function AddCartButton({
  ...
  return (
    <ConfigProvider theme={customTheme}>
      <Button
        type="primary"
        className="flex justify-center items-center hover:scale-95 transition-all ease-in-out w-full"
        loading={addToCartLoading}
        onClick={handleCllick}
      >
        <div className="inline-flex items-center mx-auto">
          <img
            src={lightMode ? cartButtonIcon : cartButtonIconDark}
            className="w-[14px] mr-[10px]"
          />
          <p className="text-black-main dark:text-dark-black-main firago-semibold text-xs leading-5 opacity-80">
            <FormattedMessage id={isInCart(productId) ? "added" : "add"} />
          </p>
        </div>
      </Button>
