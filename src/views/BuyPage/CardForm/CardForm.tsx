  ...
  return (
    <div className="w-full flex custom-container">
      <div className="w-full">
        <div
          className="inline-flex items-center mb-[30px] cursor-pointer"
          onClick={() => {
            setOnAddress(true);
          }}
        >
          <img
            alt="go back icon"
            src={lightMode ? goBackIcon : goBackIconDark}
            className="h-full mr-[20px]"
          />
          <h2 className="firago-bold text-base leading-[19px] text-black-08 dark:text-dark-black-8">
          ...
          >
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: <FormattedMessage id="please.enter.cardnumber" />,
                },
                {
                  len: 19,
                  message: <FormattedMessage id="card.number.full" />,
                },
              ]}
              getValueFromEvent={(e) => {
            ...
            >
              <Input
                type="text"
                placeholder={formatMessage({ id: "card.number" })}
                maxLength={19}
              />
            </Form.Item>
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: <FormattedMessage id="please.enter.fullname" />,
                },
              ]}
            >
              <Input
            ...
            <div className="flex justify-between">
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: <FormattedMessage id="please.enter.expiry" />,
                  },
                  {
                    len: 5,
                    message: <FormattedMessage id="please.enter.full.date" />,
                  },
                  () => ({
                    validator(_, value) {
                      if (!value) {
                      ...
                      if (enteredYear < currentYear) {
                        ...
                        enteredYear +=
                          Math.floor((currentYear - 100) / 100) * 100;
                      } else {
                        enteredYear += Math.floor(currentYear / 100) * 100;
                      }
                      ...
                      if (enteredMonth < 1 || enteredMonth > 12) {
                        return Promise.reject(
                          new Error(formatMessage({ id: "valid.month.01-12" }))
                        );
                      }
                      ...
                      if (
                        ...
                        enteredYear > currentYear + 20
                      ) {
                        return Promise.reject(
                          new Error(formatMessage({ id: "valid.year.24" }))
                        );
                      }
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: <FormattedMessage id="please.enter.cvc" />,
                  },
                  {
                    min: 3,
                    max: 4,
                    message: <FormattedMessage id="cvc.34.digits" />,
                  },
                ]}
                getValueFromEvent={(e) => {
                  const value = e.target.value.replace(/\D/g, "");
