...
export default function Products() {
  ...
  const {
    pageSize,
    /*minPrice, maxPrice, isForSale, setMinPrice, setMaxPrice, setIsForSale,*/ defaultMinPrice,
    defaultMaxPrice,
  ...
  const { products, productsLoading, totalProducts, setProducts } =
    useGetProducts({
  ...
  const productCartSkeletons = Array.from({ length: pageSize }, (_, index) => (
  ...
  const sortProducts = useCallback(() => {
    switch (sortOrder) {
      ...
      case SortEnum.DEFAULT:
        ...
        ]); // sort how the products are first set in the array (most recent at the beginning)
        break;
      default:
        break;
    }
        ...
        <div className="w-full flex flex-col lg:flex-row lg:justify-between items-start lg:items-center">
          ...
          >
            <img
              ...
              alt="go back icon"
              className="h-full mr-[22px]"
            />
            <h2 className="firago-bold text-base leading-[19px] text-black-main dark:text-dark-black-main">
              {category}
            </h2>
          </div>
          {/* desktop sort */}
        ...
        <div>
          <div>
            <div className="w-full flex">
              ...
              {productsLoading ? (
                <div className=" w-full lg:pl-8 lg:w-[760px] xl:w-[800px] grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-6 ml-auto">
                  {productCartSkeletons}
                </div>
              ) : products.length > 0 ? (
                <ProductsList
                  products={products}
                ...
                <h2 className="pl-8 text-black-main dark:text-dark-black-main firago-semibold text-lg leading-[20px]">
